                                        ...
                                        <div class="col-md-12 mt-3">
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 mt-3">
                                                    ...
                                                    <select name="drivers" id="drivers" class="js-example-basic-single2 form-control form-control-solid">
                ...
                <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                    <div id="kt_modal_new_target_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="#">
                        <div class="mb-13 text-center">
                            <h1 class="mb-3">Assign Orders To Driver</h1>
                        </div>
                        <form  method="post" id="InsertForm" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class=" col-md-12 mb-8 fv-row fv-plugins-icon-container">
                                    <label class="required mb-2">Drivers</label>
                                    <select name="assign_driver" id="assign_driver" class="js-example-basic-single2 form-control form-control-solid">
                                        <option value="" selected>Choose a Driver</option>
                                        @foreach($DriverData as $Driver)
                                            <option value="{{$Driver->id}}">{{$Driver->name}}</option>
                                        @endforeach
                        ...
                        <form  method="post" id="UpdateForm" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class=" col-md-12 mb-8 fv-row fv-plugins-icon-container">
                                    ...
                                    <select name="driver_id" id="driver_id" class="js-example-basic-single2 form-control form-control-solid">
                                        <option value="" selected>Choose a Driver</option>
                                        @foreach($DriverData as $Driver)
                                            <option value="{{$Driver->id}}">{{$Driver->name}}</option>
                                        @endforeach
                            ...
                            <div class="text-center">
                                <button type="button" class="btn btn-light me-3" data-bs-dismiss="modal">Cancel</button>
                                <button type="button" onclick="update_item()" id="kt_modal_new_target_submit" class="btn btn-primary">
...
@push('js')
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#drivers,#status,#state,#area,#start_date,#end_date').on('change', function () {
                table.ajax.reload();
            });
        });
    ...
    <script>
        ...
        $('body').on('click', '.edit', function () {
            ...
            $.ajax({
                url: "{{Route('get_company_order_details')}}",
                type: "GET",
                data: { id: id },
                cache: false,
                success: function (response) {
                    if (response.order) {
                        ...
                        $('#driver_id').val(order.driver_id).trigger('change');
                    }
                },
                error: function () {
                    alert("Error fetching order details");
        ...
        function update_item() {
            ...
            $("#kt_modal_new_target_submit_edit").text("Loading").prop("disabled", true);
            $.ajax({
                url: "{{route('update_company_orders')}}",
                type: "POST",
                data: form_Data,
                contentType: false,
                cache: false,
                ...
                success: function (dataResult) {
                    ...
                    $("#kt_modal_new_target_submit_edit").text("Update").prop("disabled", false);

                    if (dataResult == 1) {
                        $('#table').DataTable().ajax.reload(null, false);
                        $('#kt_modal_new_target_edit').modal('hide');
                        toastr.success('Orders assigned successfully.');
                        $("#UpdateForm")[0].reset();
                        $('#assignOrdersBtn').hide();
                    } else if (dataResult == 2) {
                        toastr.error('Select a Driver.');
                    } else {
        ...
        function insert_item() {
            ...
            $.ajax({
                url: "{{route('assign_company_orders')}}",
                type: "POST",
                data: form_Data,
                contentType: false,
                cache: false,
