    ...
    <div class="dashboard-container">
        ...
        <div class="row align-items-center mb-4">
            <div class="col-md-8 dashboard-mobile-responsiveness">
                <h1 class="mb-1" style="color: var(--text-primary); font-weight: 700; font-size: 2rem;}">UAE Dropshipper Dashboard
                </h1>
                <p style="color: var(--text-secondary); margin: 0;">Welcome back! Here's what's happening with your store
                    today.</p>
            </div>
                    ...
                    <div class="col-lg-6">
                        <div class="analytics-box earnings-analytics">
                            <div class="text-center">
                                ...
                                <p style="color: var(--text-secondary); font-size: 0.875rem;">Revenue for selected period
                                </p>
                                <h2 class="text-success fw-bold">{{ number_format($totalEarnings ?? 0, 2) }}</h2>
                                <small style="color: var(--text-secondary);">
                                    {{ ($earningsGrowth ?? 0) >= 0 ? '+' : '' }}{{ number_format($earningsGrowth ?? 0, 1) }}%
                                    increase
                                </small>
                            </div>
                        </div>
        ...
        <div class="row g-4 mb-4">
                ...
                <div class="management-card card shadow-sm">
                    <div class="d-flex align-items-end pb-4">
                        <h5 class="mb-3 card-title text-primary fw-bold col-9">
                            <i class="fas fa-credit-card text-primary me-2"></i> Transactions
                        </h5>
                        <div class="text-end col-3">
                            <a href="/admin/all-product-stocks" class="btn btn-primary btn-sm">View All</a>
                        </div>
                    </div>
                    <div class="management-item bg-success-light mb-3 p-3 rounded">
                        <span style="color: var(--text-primary); font-weight: 500; font-size: 0.875rem;">Total Amount In</span>
                        <span class="text-success fw-bold">{{ number_format($totalAmountIn, 2) }} AED</span>
                    </div>
                    <div class="management-item bg-primary-light mb-3 p-3 rounded">
                        <span style="color: var(--text-primary); font-weight: 500; font-size: 0.875rem;">Total Wallet</span>
                        <span class="text-primary-custom fw-bold">{{ number_format($totalWallet, 2) }} AED</span>
                    </div>
                    <div class="management-item bg-warning-light p-3 rounded">
                        <span style="color: var(--text-primary); font-weight: 500; font-size: 0.875rem;">Total Amount Out</span>
                        <span class="text-warning fw-bold">{{ number_format($totalAmountOut, 2) }} AED</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 col-md-6">
                <div class="management-card card shadow-sm">
                    <div class="d-flex align-items-end pb-4">
                        <h5 class="mb-3 card-title text-primary fw-bold col-9">
                            <i class="fas fa-box text-primary me-2"></i> Inventory Stocks
                        </h5>
                        <div class="text-end col-3">
                            <a href="/admin/all-product-stocks" class="btn btn-primary btn-sm">View All</a>
                        </div>
                    ...
                    <div class="management-item bg-primary-light mb-3 p-3 rounded">
                        <span style="color: var(--text-primary); font-weight: 500; font-size: 0.875rem;">Total Products</span>
                        <span class="text-primary-custom fw-bold">{{ number_format($totalProducts) }}</span>
                    </div>
                    <div class="management-item bg-danger-light mb-3 p-3 rounded">
                        <span style="color: var(--text-primary); font-weight: 500; font-size: 0.875rem;">Low Stock</span>
                        <span class="text-danger fw-bold">{{ $lowStock }}</span>
                        @if ($lowStock > 0)
                            <button type="button" class="btn btn-sm btn-outline-danger ms-2" data-bs-toggle="modal" data-bs-target="#lowStockModal" style="margin-right: 6px; background: #000000; padding: 0px 10px 0px; color: #ffffff;">
                                View
                            </button>
                        @endif
                    </div>
                    <div class="management-item bg-info-light p-3 rounded">
                        <span style="color: var(--text-primary); font-weight: 500; font-size: 0.875rem;">Out of Stock</span>
                        <span class="text-info fw-bold">{{ $outOfStock }}</span>
                        @if ($outOfStock > 0)
                            <button type="button" class="btn btn-sm btn-outline-info ms-2" data-bs-toggle="modal" data-bs-target="#outOfStockModal" style="margin-right: 6px; background: #ff0000; padding: 0px 10px 0px; color: #ffffff;">
                                View
                            </button>
                        @endif
                    </div>
                                ...
                                <tbody>
                                    @foreach ($lowStockProducts as $product)
                                        <tr>
                                            ...
                                            <td>{{ $product->variation_name ? $product->variation_name . ': ' . $product->variation_value : 'N/A' }}
                                            </td>
            ...
            <div class="col-lg-6 col-md-12 mb-4">
                <div class="management-card card shadow-sm">
                    ...
                    @forelse($topAreas as $index => $area)
                        <div
                            class="management-item d-flex justify-content-between align-items-center p-3 mb-3 rounded @if ($index == 0) bg-warning-light @elseif($index == 1) bg-success-light @else bg-purple-light @endif">
                            <span class="fw-medium" style="color: var(--text-primary); font-size: 0.875rem;">
                                {{ ucfirst($area->state ? $area->state->state : 'N/A') }} - {{ ucfirst($area->area) }} -
                                Orders = {{ $area->order_count }}
                            </span>
                            <span
                                class="badge @if ($index == 0) bg-warning @elseif($index == 1) bg-success @else bg-purple @endif fw-bold"
                                style="color: black">
            ...
            <div class="modal fade" id="topAreasModal" tabindex="-1" aria-labelledby="topAreasModalLabel"
                aria-hidden="true">
                                    ...
                                    <tbody>
                                        @forelse($topTenAreas as $index => $area)
                                            <tr>
                                                ...
                                                <td>{{ $area->delivered_count }}
                                                    ({{ number_format(($area->delivered_count / ($area->order_count ?: 1)) * 100, 1) }}%)
                                                </td>
                                                <td>{{ $area->cancelled_count }}
                                                    ({{ number_format(($area->cancelled_count / ($area->order_count ?: 1)) * 100, 1) }}%)
                                                </td>
                                                <td>{{ number_format(($area->order_count / ($totalOrders ?: 1)) * 100, 1) }}%
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="7" class="text-center">No orders found for this period.
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
            ...
            <div class="modal fade" id="topStatesModal" tabindex="-1" aria-labelledby="topStatesModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                                    ...
                                    <tbody>
                                        @forelse($topTenStates as $index => $state)
                                            <tr>
                                                ...
                                                <td>{{ $state->delivered_count }}
                                                    ({{ number_format(($state->delivered_count / ($state->order_count ?: 1)) * 100, 1) }}%)
                                                </td>
                                                <td>{{ $state->cancelled_count }}
                                                    ({{ number_format(($state->cancelled_count / ($state->order_count ?: 1)) * 100, 1) }}%)
                                                </td>
                                                <td>{{ number_format(($state->order_count / ($totalOrders ?: 1)) * 100, 1) }}%
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="6" class="text-center">No orders found for this period.
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                ...
                <div class="management-card card shadow-sm">
                    <div class="d-flex align-items-end pb-4">
                        ...
                        @if ($topProducts->count() > 0)
                            <div class="text-end col-3">
                                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#topProductsModal">
                                    View All
                                </button>
                            </div>
                        @endif
                                    ...
                                    <tbody>
                                        @forelse($topTenProducts as $index => $product)
                                            <tr>
                                                ...
                                                <td>{{ $product->delivered_quantity }}
                                                    ({{ number_format(($product->delivered_quantity / ($product->total_quantity ?: 1)) * 100, 1) }}%)
                                                </td>
                                                <td>{{ $product->cancelled_quantity }}
                                                    ({{ number_format(($product->cancelled_quantity / ($product->total_quantity ?: 1)) * 100, 1) }}%)
                                                </td>
                                                <td>{{ number_format(($product->delivered_quantity / ($totalDeliveredQuantity ?: 1)) * 100, 1) }}%
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="8" class="text-center">No products delivered in this
                                                    period.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
            ...
            <!-- Modal for Top 10 Ordered Products -->
            <div class="modal fade" id="topProductsModal" tabindex="-1" aria-labelledby="topProductsModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                                    ...
                                    <tbody>
                                        @forelse($topTenProducts as $index => $product)
                                            <tr>
                                                ...
                                                <td>{{ number_format(($product->total_quantity / ($totalOrders ?: 1)) * 100, 1) }}%
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="9" class="text-center">No products ordered in this period.
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                        ...
                        @foreach ($orders as $order)
                            <tr style="border-bottom: 1px solid var(--border-color);">
                                <td class="fw-semibold" style="padding: 1rem;">
                                    {{ $order->sub_seller ? $order->sub_seller->unique_id . '-' . $order->id : '#ORD-' . $order->id }}
                                </td>
                                <td style="padding: 1rem;">{{ $order->customer_name }}</td>
                                <td style="padding: 1rem;">
                                    <span
                                        class="status-badge status-{{ strtolower(str_replace(' ', '', $order->status)) }}">{{ $order->status }}</span>
                                </td>
