...
@if ($order->status != 'Pending')
    <script>window.location.href = "{{ route('all_orders') }}";</script>
@endif
...
@push('js')
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '.item-quantity', function() {
                ...
                $.ajax({
                    url: "{{ route('orders.update_item', $order->id) }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        item_id: itemId,
    ...
    if (productType === 'variable') {
        $.ajax({
            url: "{{ route('get_product_variations') }}",
            type: "GET",
...
$('#addItemBtn').click(function () {
    ...
    $.ajax({
        url: "{{ route('orders.add_item', $order->id) }}",
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
...
// Remove item with inventory feedback
            $(document).on('click', '.remove-item', function() {
                ...
                $.ajax({
                    url: "{{ route('orders.remove_item', $order->id) }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        item_id: itemId
